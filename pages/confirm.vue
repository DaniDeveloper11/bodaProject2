<template>
    <div class="flex flex-col  px-4 gap-4 my-10 w-screen dark:text-white">
        <div class="flex justify-end -mb-20">
            <NuxtImg :src="weddingConfig.images.flowerCorner" class="-scale-y-100 size-40"></NuxtImg>
        </div>

        <h1 class="text-3xl font-dancing mb-4 dark:text-white">{{ weddingConfig.texts.confirmPage.title }}</h1>
        <p v-if="isFamily" class="text-md mb-6 mx-auto text-center text-xs px-3">
            {{ weddingConfig.texts.confirmPage.familyMessage }}
        </p>
        <p v-else class="text-md mb-6 mx-auto text-center text-xs px-3">
            {{ weddingConfig.texts.confirmPage.companionMessage }}
        </p>
        <p></p>
        ...
        <div class="px-6 sm:px-6 lg:px-8 mx-auto">
            <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                    ...
                    <p class="mt-2 text-sm text-gray-700 dark:text-white">Favor de confirmar antes del <span class="font-semibold"> {{ weddingConfig.date.rsvpDeadline }}</span></p>
                </div>
            </div>
                                ...
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th scope="col"
                                            ...
                                            {{ weddingConfig.texts.confirmPage.nameLabel }}</th>
                                    ...
                                    <tr v-if="!isFamily">
                                        ...
                                        <td class="py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
                                            ...
                                            <p v-if="guest.confirmed" class=" text-xs font-thin text-green-500">
                                                {{ weddingConfig.texts.confirmPage.confirmedLabel }}</p>
                                        </td>
        ...
        <div class="flex justify-end mt-6 mx-auto px-4 sm:px-20 w-full max-w-2xl">
            <button
                ...
                <NuxtImg class="size-11 absolute bottom-0 right-0 -scale-y-100 -scale-x-100 pointer-events-none" :src="weddingConfig.images.disenoFloral3"></NuxtImg>
                {{ weddingConfig.texts.confirmPage.submitButton }}
            </button>
        </div>
        ...
        <div class="mt-8 rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50/60 dark:bg-red-950/20 px-5 py-4 max-w-lg">
            <p class="text-gray-700 dark:text-gray-200 text-base font-semibold">
                {{ weddingConfig.texts.confirmPage.adultsOnly }}
            </p>
            <a class="text-xs text-brand-sage dark:text-brand-gold underline mt-1 inline-block" :href="weddingConfig.texts.confirmPage.adultsOnlyLink">
                ¿Por qué? →
            </a>
        </div>

        <div class="mt-6">
            <p class="text-gray-600 dark:text-gray-300 text-sm">{{ weddingConfig.texts.confirmPage.footer }}</p>
            <p class="text-gray-600 dark:text-gray-300 text-sm">{{ weddingConfig.texts.confirmPage.footerThanks }}</p>
        </div>
        ...
        <div class="flex justify-start mt-8">
            <button @click="navigateToHome"
                ...
                <span>{{ weddingConfig.texts.confirmPage.backButton }}</span>
            </button>
        </div>
        <ModalConfirmAlert v-model="showModal" :title="weddingConfig.texts.confirmPage.modalTitle"
            :message="weddingConfig.texts.confirmPage.modalMessage" @confirm="handleConfirm"></ModalConfirmAlert>
...
const handleConfirm = async () => {
    ...
    if (guestStore.error) {
        alert(weddingConfig.texts.confirmPage.errorMessage)
    } else {
        alert(weddingConfig.texts.confirmPage.successMessage)
        navigateToHome();
    }
}
