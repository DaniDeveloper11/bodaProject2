<template>
    <div class="my-20 max-w-6xl mx-auto relative">
        ...
        <div class="w-full flex justify-end my-8">
            <NuxtImg :src="config.images.simpleFlower" class="-scale-y-100 size-24 opacity-70"></NuxtImg>
        </div>
        <div class="flex flex-col  justify-center my-10">
            <h2 class="text-2xl font-dancing text-brand-charcoal mb-4 md:text-4xl">{{ config.texts.confirmation.title }}</h2>
            <div class="flex flex-col w-full gap-6 mx-auto max-w-2xl">
                <div>
                    <label for="email" class="block text-sm/6 font-medium text-brand-charcoal">{{ config.texts.confirmation.codeLabel }}</label>
                    <div class="mt-2">
                    ...
                    <p class="mt-2 text-sm text-brand-charcoal/60" id="email-description">{{ config.texts.confirmation.codeHelp }}
                    </p>
